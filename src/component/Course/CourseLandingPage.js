...
import {
  courseDataAction,
  courseLandingPageDataAction,
  coursePaymentAction,
  coursePaymentStatusAction,
  enrollCourse,
  getCourseInfoByBatchId,
...
const CourseLandingPage = () => {
  ...
  const { loading, courseLandingPageData } = useSelector(
    (state) => state.courseLandingPage
  );
  const {
  ...
  const { coursePayment, coursePaymentStatus } = useSelector(
    (state) => state.payment
  );
  ...
  const instructorImage =
    course.instructor_section?.instructor_image || "/instructor.png";
  ...
  return (
    <div className="CourseLandingPage_container">
      ...
      {/* Confirm modal */}
      <div
        className="confirmModal_container"
        style={confirmModal ? { display: "flex" } : { display: "none" }}
      >
        <h1 className="confirmModal_heading">
      ...
      {/* Payment modal */}
      <div
        className="pymentModal_container_parent"
        style={paymentModal ? { display: "flex" } : { display: "none" }}
      >
        <div className="pymentModal_container">
          <IoIosCloseCircle
            className="payment_close_btn"
            onClick={() => setPaymentModal(false)}
          />
          <div
            className="paymentModal_image"
            style={{ backgroundImage: `url(${paymentImage})` }}
          ></div>
          <h1 className="paymentModal_heading">
            {paymentCourseData.course_name}
          </h1>
          <h1 className="paymentModal_price">
            Course price - ₹{paymentCourseData.batch_price}
          </h1>
          <p className="paymentModal_description">
            {paymentCourseData.description}
          </p>
          <button className="paymentModal_btn" onClick={handlePayment}>
            Pay Now
          </button>
        </div>
      </div>
        ...
        <div className="hero-content">
          <div className="hero-left">
            <h1 className="hero-title">
              {course.hero_section?.hero_title || "Default Hero Title"}
            </h1>
            {console.log("herooo", course)}
          ...
          <div className="hero-right">
            <img className="hero-image" src={heroImage} alt="Course Hero" />
          </div>
        </div>
      </section>
      ...
      <div className="course-details-wrapper">
        <section className="what-you-get-section">
          ...
          {course.what_you_get_section.points &&
          course.what_you_get_section.points.length > 0 ? (
        ...
        <aside className="instructor-section">
          ...
          <div className="instructor-profile">
            ...
            <div className="instructor-info">
              ...
              <div className="instructor-designation">
                {course.instructor_section?.instructor_designation ||
                  "Default Instructor Designation"}
              </div>
              <div className="instructor-description">
                {course.instructor_section?.instructor_description ||
                  "Default Instructor Description, Default Instructor Description, Default Instructor Description, Default Instructor Description, Default Instructor Description, Default Instructor Description, Default Instructor Description"}
              </div>
            </div>
          </div>
        </aside>
      ...
      <div className="course_curriculum">
        ...
        <div className="course_curriculum_right">
          <div
            className="course_curriculum_img"
