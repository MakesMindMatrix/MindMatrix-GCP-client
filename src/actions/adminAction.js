...
export const getAllUsers = (paginateData, filterQuery) => async (dispatch) => { 
    ...
    try {
        ...
        const { data } = await axios.get(`/api/v1/get-all-users?page=${paginateData.page}&limit=${paginateData.limit}&college=${filterQuery.college}&branch=${filterQuery.branch}&semester=${filterQuery.semester}`, config)
...
export const getBatchList = () => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`/api/v1/getBatch`, config)
...
export const enrolledListAction = (batch_id) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post(`/api/v1/enrolled-student`, batch_id, config)
        console.log(data)
...
export const unEnrollAction = () => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`/api/v1/get-all-users`, config)
...
export const getAllPaymentAction = () => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`/api/v1/all-payment`, config)
