...
export const courseLandingPageDataAction = () => async (dispatch) => {
    try {
        dispatch({ type: ALL_COURSE_LANDING_PAGE_DATA_REQUEST })

        const config = { headers: { "Content-Type": "application/json" } }

        const { data } = await axios.get(`${BASE_URL}/api/v1/course-info`, config)

        dispatch({ type: ALL_COURSE_LANDING_PAGE_DATA_SUCCESS, payload: data })
    } catch (error) {
        dispatch({ type: ALL_COURSE_LANDING_PAGE_DATA_FAIL, payload: error.response.data.message })
    }
}
